    ...
    <div className="relative min-h-screen w-full flex items-center justify-center bg-[radial-gradient(circle_at_center,#1a1a1a_0%,#000000_100%)] overflow-hidden px-6">
      ...
      <div className="relative z-10 w-full max-w-[440px] space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-4xl font-bold text-white uppercase tracking-tighter italic">VAULT ACCESS</h1>
          <p className="text-label-caps text-white/40 tracking-[0.3em] uppercase">Enter your credentials to unlock</p>
        </div>
        ...
        <div className="bg-white/[0.02] backdrop-blur-3xl border border-white/10 rounded-2xl p-8 md:p-10 space-y-8">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormItem className="space-y-2">
                    ...
                    <FormControl>
                      <Input 
                        ...
                        className="bg-black/50 border-white/10 h-14 text-white focus:border-white transition-all duration-500 rounded-xl" 
                        {...field} 
                      />
                    </FormControl>
              ...
              <button 
                ...
                disabled={isLoading}
              >
                {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Unlock Vault"}
              </button>
            </form>
          </Form>
          ...
          <div className="grid grid-cols-1 gap-4">
            <button 
              ...
              className="h-14 border border-white/10 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-white/5 transition-all flex items-center justify-center gap-3"
            >
              {isMagicLinkLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />}
              Send Magic Link
            </button>
            <button 
        ...
        <div className="flex flex-col items-center gap-4 text-[10px] tracking-widest uppercase font-bold">
          <Link href="/auth/signup" className="text-white/40 hover:text-white transition-colors underline underline-offset-8">
            Create Master Account
          </Link>
          <Link href="/auth/forgot-password" className="text-white/20 hover:text-white transition-colors">
            Identity Recovery
          </Link>
        </div>
      </div>
